...
def changing_tag_in_dict(info_tag,format_tag,dict_file):
    """ changing the common tag in the the dit file provided, using the tag"""
...
def merging_all_the_common_variant(info_tag,format_tag,common_dict_1,common_dict_2,file_name_1,file_name_2):
    ...
    common_dict_1 = changing_tag_in_dict(changing_info_tag,changing_format_tag,common_dict_1)
    ...
    changing_format_tag = dict(zip(format_tag, new_format_tag))
    common_dict_2 = changing_tag_in_dict(changing_format_tag,changing_format_tag,common_dict_2)
    ...
    for key, value in common_dict_1.items():
        ...
        temp_format_dict_1 = { }
        temp_format_dict_2 = { }
        temp_format_detial_list_1 = [ ]
        temp_format_detial_list_2 = [ ]
        temp_format_list_1 = [ ]
        temp_format_list_2 = [ ]
        ...
        new_value.insert(2, (';'.join(set(common_dict_1[key][2].split(';')) | set(common_dict_2[key][2].split(';')))))
        temp_format_list = (set(common_dict_1[key][3].split(':')) | set(common_dict_2[key][3].split(':')))
        ...
        temp_format_detial_list_1 = common_dict_1[key][4].split(':')
        temp_format_detial_list_2 = common_dict_2[key][4].split(':')

        temp_format_dict_1 = dict(zip(temp_format_list_1,temp_format_detial_list_1))
        temp_format_dict_2 = dict(zip(temp_format_list_2,temp_format_detial_list_2))
        ...
        temp_format_dict_1 = dict(sorted(temp_format_dict_1.items(), key=lambda pair: temp_format_list.index(pair[0])))
        temp_format_dict_2 = dict(sorted(temp_format_dict_2.items(), key=lambda pair: temp_format_list.index(pair[0])))
...
def variant_file_to_dict(file_variant):
    """Modifing the variant tabular into dict where key value is "chr_pos_ref_alt" and the rest of the
    information such as info, format are store as value"""
...
def header_seperation(vcf_file):
    """This function mainly seperate into two divisions """
...
def reading_file(input_vcf_file1,input_vcf_file2, output_vcf_file):
    """This function is read file and seperate it into info header and variant file"""

    print("** Reading the " +input_vcf_file1 + " file **")
    header_file1, file_info_header_1, file_variant = header_seperation(input_vcf_file1)
    vcf_file_1_dict = variant_file_to_dict(file_variant)
    ...
    vcf_file_2_dict = variant_file_to_dict(file_variant)  # function for convertion of variant list to dict with proper key format
    ...
    info_tag,format_tag = identifying_common_info_format_tag(list_vcf_dict_1,list_vcf_dict_2)  # function for indentification of common tags
    common_dict  = merging_all_the_common_variant(info_tag,format_tag,common_dict_1,common_dict_2,file_name_1,file_name_2) #merging the common dict
    ...
    print("The unique number of variant" + input_vcf_file1 + " is :", len(unique_dict_1))
    print("The unique number of variant" + input_vcf_file2 + " is :", len(unique_dict_2))
    print("The common number of variant in " + input_vcf_file1 + "and " + input_vcf_file2 + " is:", len(common_dict))
...
def main(argv):
    if argv != []:
        try:
            opts, args = getopt.getopt(argv, "hf:v:o:", ["help", "file1=","file2=", "ofile="])
        ...
        for opt, arg in opts:
            ...
            if opt in ("-f", "--file1"):
                ...
                assert False
        reading_file(input_vcf_file1,input_vcf_file2, output_vcf_file)
    else:
        print(
